...
class ResourceService {
    async getMenuOptions() {
        try {
            ...
            const filtered = products?.filter((product: IProduct) => product.menuPosition ? true : false)
            ...
            const menuOptions = filtered.map((product: IProduct) => (
                {
                    name: product.name, url: `product?product_ref=${product.productRef}`,
                }
            ))
    ...
    async getMosaicOptions() {
        try {
            ...
            const mosaicOptions = products?.map((product: { mainImage: string, name: string, productRef: string }) => (
                {
    ...
    async getRecommendations(productRef: string) {
        try {
            ...
            const filtered = products?.filter((product: IProduct) => product.productRef === productRef ? false : true)

            const recomendations = filtered.map((product: IProduct) => (
                { src: product.mainImage, alt: product.name, url: `product?product_ref=${product.productRef}`}
            )
            )
